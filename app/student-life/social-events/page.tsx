...
const HERO_IMAGE_URL = "/images/student-life-social-events-hero.png";
export default async function SocialEventsPage() {
  const section = getStudentLifeSection("social-events");
  if (!section) return null;
  const body = await fetchPageMainContent(section.href, section.title, section.body);

  return (
    <div className="min-h-screen bg-stone-50 text-slate-900">
      <PageHero src={HERO_IMAGE_URL} alt="イベント" title={section.title} />
