...
const HERO_IMAGE_URL = "/images/student-life-social-clubs-hero.png";
export default async function SocialClubsPage() {
  const section = getStudentLifeSection("social-clubs");
  if (!section) return null;
  const body = await fetchPageMainContent(section.href, section.title, section.body);

  return (
    <div className="min-h-screen bg-stone-50 text-slate-900">
      <PageHero src={HERO_IMAGE_URL} alt="ガラス張りの明るいロビーに大きなINSEADのオブジェが置かれた様子" title={section.title} />
