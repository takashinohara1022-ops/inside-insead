...
function AuthorLabel({
  ...
  if (!href) return <>{text}</>;
  return (
    <Link href={href} className="text-[#005543] underline underline-offset-2">
      {text}
    </Link>
  );
}
...
export function StudentsBlogBoard({
  ...
  const handleLikeUpdate = useCallback((postId: string, data: { count: number; liked: boolean }) => {
          ...
          >
            ...
            <div className="mt-4 flex justify-start">
              <LikeButton
                    ...
                    initialCount={likesByPostId[selectedPost.id]?.count ?? 0}
                    initialLiked={likesByPostId[selectedPost.id]?.liked ?? false}
                    onUpdate={handleLikeUpdate}
                  />
            </div>
