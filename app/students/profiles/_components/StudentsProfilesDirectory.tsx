...
type AccordionKey = "basic" | "application" | "why" | "message" | "blog";
...
function getCoffeeChatWindow(classLabel: string): { start: Date; end: Date } | null {
  ...
  if (intake === "D") {
    const admissionYear = graduationYear;
    return {
      start: new Date(admissionYear, 0, 1, 0, 0, 0, 0),
      end: new Date(admissionYear, 10, 30, 23, 59, 59, 999),
    };
  }
...
function getCoffeeChatAvailability(classLabel: string, now: Date): CoffeeChatAvailability {
  ...
  const isOpen = today >= window.start && today <= window.end;
  if (isOpen) {
    return {
      isOpen: true,
    };
...
function parseProfiles(rows: SheetRow[]): StudentProfile[] {
  return rows.map((row, index) => {
    ...
    const whyCategories = dedupeTags(
      splitMultiValue(
        getByHeaderMatch(row, [
          "Why INSEAD? 判断軸カテゴリー",
          "Why INSEAD？ 判断軸カテゴリー",
    ...
    >
      ...
      <div className="mt-4 space-y-3">
        ...
        {coffeeChatAvailability.isOpen ? (
          ...
          >
            {profile.initials}とのコーヒーチャットを申込む
          </Link>
        ) : null}
      </div>
    </article>
...
export function StudentsProfilesDirectory({
  ...
  const coffeeChatByProfileId = useMemo(() => {
    const map = new Map<string, CoffeeChatAvailability>();
    profiles.forEach((profile) => {
      map.set(profile.id, getCoffeeChatAvailability(profile.classLabel, today));
    });
    return map;
  }, [profiles, today]);
  ...
  const filteredProfiles = useMemo(() => {
    ...
    const tagFiltered =
      selectedOtherTagKeys.length === 0
        ? yearFiltered
        : yearFiltered.filter((profile) =>
      selectedOtherTagKeys.every((tagKey) => profile.filterTagKeys.includes(tagKey)),
    );
    if (!coffeeChatOnly) return tagFiltered;
    return tagFiltered.filter((profile) => coffeeChatByProfileId.get(profile.id)?.isOpen);
  }, [profiles, selectedYearKeys, selectedOtherTagKeys, coffeeChatOnly, coffeeChatByProfileId]);
  ...
  const toggleCoffeeChatOnly = () => {
    resetPaginationState();
    setCoffeeChatOnly((prev) => !prev);
  };
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-4 sm:p-5">
        ...
        <div className="mt-5 border-t border-neutral-100 pt-4">
          <p className="text-sm font-semibold text-slate-800">コーヒーチャット</p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={toggleCoffeeChatOnly}
              className={`rounded-full px-2.5 py-1 text-xs font-medium transition-colors ${
                coffeeChatOnly
                  ? "bg-emerald-800 text-white"
                  : "bg-green-100 text-green-800 hover:bg-green-200"
        ...
        <div className="mt-5 border-t border-neutral-100 pt-4">
          ...
          <div className="mt-3 flex min-h-8 flex-wrap items-center gap-2">
            {selectedOtherTags.length > 0 || coffeeChatOnly ? (
              <>
                ...
                {coffeeChatOnly ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-1 text-sm text-emerald-800">
                    #コーヒーチャット可能
                    <button
                      type="button"
                      aria-label="コーヒーチャット可能を解除"
                      onClick={toggleCoffeeChatOnly}
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full text-emerald-800/80 hover:bg-emerald-200"
                    >
                      ×
